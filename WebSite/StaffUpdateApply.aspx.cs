...
public partial class _Default : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            conn.Open();
            SqlCommand cmd = new SqlCommand("staff_view", conn);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.Add("@staff_id", SqlDbType.VarChar).Value = str;
            ...
            if (reader.HasRows)
            {
                ...
                if (!reader.GetValue(15).ToString().Equals(""))
                    Image1.ImageUrl = HiddenField4.Value = "Uploads/Staff/" + reader.GetValue(15).ToString();
                else
                    Image1.ImageUrl = "";
                ...
                if (!reader.GetValue(13).ToString().Equals(""))
                    TextBox34.Text = reader.GetValue(13).ToString().Remove(reader.GetValue(13).ToString().IndexOf(' '));
                TextArea2.InnerText = reader.GetValue(17).ToString();
            }
            conn.Close();
        }
    }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        {
            System.IO.File.Move(Server.MapPath(Image1.ImageUrl.ToString()), Server.MapPath("~/PicUpdateApply/Staff/" + Image1.ImageUrl.ToString().Substring(Image1.ImageUrl.ToString().LastIndexOf('/') + 1)));
            //Image1.ImageUrl = Server.MapPath("~/PicUpdateApply/Staff/" + HiddenField1.Value.Substring(HiddenField1.Value.LastIndexOf('/') + 1));
        ...
        string xmlPath = MapPath("~/staffupdate.xml");
        ...
        if (System.IO.File.Exists(xmlPath))
        {
            ...
            XmlNode updateStoreNode = doc.SelectSingleNode("staff");
        ...
        else
        {
            ...
            XmlNode updateStoreNode = doc.CreateElement("staff");
            XmlNode updateNode = CreateUpdateNode(doc);
    ...
    protected XmlNode CreateUpdateNode(XmlDocument doc)
    {
        ...
        staffid.InnerText = Session["user_id"].ToString();
        updateNode.AppendChild(staffid);
        ...
        if (!Image1.ImageUrl.ToString().Equals(""))
            imageurl.InnerText = "PicUpdateApply/Staff/" + Image1.ImageUrl.ToString().Substring(Image1.ImageUrl.ToString().LastIndexOf('/') + 1);
        else
